...
router.post('/', async (req, res) => {
  try {
    ...
    if (category === 'inquilino' && Object.keys(extra).length) {
      rentalDetails = ` | Localidad: ${extra.localidad||'-'}. Zona: ${extra.zona||'-'}. Alquiler: ${extra.alquiler_desde||'-'} a ${extra.alquiler_hasta||'-'}. Tipo: ${extra.tipo_propiedad||'-'}. Habitaciones: ${extra.habitaciones||'-'}. m²: ${extra.m2||'-'}. Grupo familiar: ${extra.grupo_familiar||'-'} personas. Ingresos: ${extra.ingresos||'-'}. Garantía: ${extra.garantia||'-'}. Mascotas: ${extra.mascotas||'-'}. Cochera: ${extra.cochera||'-'}. Fecha de mudanza deseada: ${extra.fecha_mudanza||'-'}.`;
    }
    if (category === 'comprador' && Object.keys(extra).length) {
      rentalDetails = ` | Localidad: ${extra.localidad||'-'}. Zona: ${extra.zona||'-'}. Presupuesto: ${extra.presupuesto_desde||'-'} a ${extra.presupuesto_hasta||'-'}. Tipo de vivienda: ${extra.tipo_vivienda||'-'}. Habitaciones: ${extra.habitaciones||'-'}. m²: ${extra.m2||'-'}. Forma de pago: ${extra.forma_pago||'-'}. Cochera: ${extra.cochera||'-'}. Antigüedad preferida: ${extra.antiguedad||'-'}. Grupo familiar: ${extra.grupo_familiar||'-'} personas. Fecha estimada de compra: ${extra.fecha_compra||'-'}.`;
    }
