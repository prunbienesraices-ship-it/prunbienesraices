...
router.post('/', requireAuth, async (req, res) => {
  ...
  const { data, error } = await supabase.from('owners').insert([{
    ...
    address: b.address || '', phone: b.phone || '', email: b.email || '', cbu: b.cbu || '', alias: b.alias || '',
    bank: b.bank || '', account: b.account || '', payment_method: b.payment_method || 'transferencia',
    iva: b.iva || '', monotributo: b.monotributo || '', commission_withholding: Number(b.commission_withholding) || 0,
    notes: b.notes || '',
  }]).select().single();
...
router.put('/:id', requireAuth, async (req, res) => {
  const b = req.body;
  const updates = {};
  ['name','dni','cuit','birthdate','marital','address','phone','email','cbu','alias','bank','account','payment_method','iva','monotributo','notes'].forEach(f=>{ if(b[f]!==undefined) updates[f]=b[f]; });
  if(b.commission_withholding!==undefined) updates.commission_withholding = Number(b.commission_withholding);
